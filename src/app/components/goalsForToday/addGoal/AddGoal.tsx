...
export default function AddGoal({ addNewGoal, setDifficulty, difficulty, goalName, setGoalName }: { 
  ...
  return (
    ...
    <div className={styles.addGoalHeader}>
     {!expanded ? <p className={styles.addGoalP}>Add A Goal</p> : <input className={styles.addGoalInput} placeholder='Goal Name' type='text'  value={goalName ?? ""} onChange={(e) => setGoalName(e.target.value)} />}
      ...
      {expanded && <div className={styles.expandedContent}>
        ...
        <GoalDifficulty easy = {easy} difficultyLevel="Difficult" diamonds = {20} />
        <GoalDifficulty easy = {easy} difficultyLevel="Impossible" diamonds = {30} />
        </div>
